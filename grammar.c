...
prod_create()
{
	return (Prod *) calloc(1, sizeof(Prod));;
}

Prod *
prod_inline_act(char *s0, ...)
{
	Prod *p = prod_create();
	va_list ap;
	va_start(ap, s0);
	for (char *sym = s0; sym; sym = va_arg(ap, char *)) {
		prod_append(p, sym);
	}
	va_end(ap);
	return p;
...
void
symbol_addprodind(Symbol *X, Prod *p, int index)
{
...
void
symbol_addprod(Symbol *X, Prod *p)
{
	symbol_addprodind(X, p, X->n);
}

static int
symbol_mustgetprodindex(Symbol *X, Prod *p)
{
	for (int i = 0; i < X->n; i++) {
...
static int
symbol_delprod(Symbol *X, Prod *p)
{
...
static bool
symbol_eq(const Symbol *X, const Symbol *Y)
{
	if (X->n != Y->n) {
		return false;
	}
...
char *
symbol_str(const Symbol *X, const Grammar *G)
{
	struct strbuilder *b = strbuilder_create();
	for (int i = 0; i < X->n; i++) {
		strbuilder_printf(b, "%s%s",
...
grammar_unleftrec_immediate(Grammar *G, const char *sym)
{
	...
	Symbol *new = symbol_create();
	Symbol *X_ = symbol_create();
...
static bool
symbolset_followsubset(const Grammar *G, char *sym, Symbol *X)
{
	for (int i = 0; i < X->n; i++) {
...
grammar_sym_LL1cond12(const Grammar *G, char *sym)
{
	Symbol *X = map_get(G->map, sym);
	assert(X != NULL);
	Symbolset *enc = prod_create(); /* symbols already encountered */
	for (int i = 0; i < X->n; i++) {
		Prod *p = X->prod[i];
...
static int
grammar_sym_prodeps(const Grammar *G, Symbol *X)
{
	for (int i = 0; i < X->n; i++) {
		Prod *p = X->prod[i];
		Symbolset *first = grammar_symbolsetfirst(G, (Symbolset *) p);
